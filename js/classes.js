...
class PlayerTransporter extends Updatable{
	constructor(x, y){
		...
		this.ship.tileArray.forEach(function(tile){
			tile.discovered = true;
			tile.visible = true;
		});
		this.arrived = false;
		this.startedDocking = false;
		this.dockTile = this.ship.getTile(this.x + TILE_SIZE * 6, 0);
	...
	update(){
		let moveSpeed = 1;
		if(!this.arrived){
			if(this.y !== normalizeToGrid(canvas.height / 2)){
				this.y += moveSpeed;
				player.y = this.y;
